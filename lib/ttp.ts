				...
				gm('../tmp/frames/*.png')
					...
					.write('../tmp/attp.gif', () => {
						ffmpeg('../tmp/attp.gif')
							...
							.save('../tmp/attp.webp')
							.on('end', () => {
								const mediaData = fs.readFileSync('../tmp/attp.webp');
								fs.unlink('../tmp/attp.gif', () => {
									fs.unlink('../tmp/attp.webp', () => {});
									fs.emptyDir('../tmp', () => {});
								});
			...
			await Promise.all([
				...
				anjay.on('close', () => {
					spawn('ffmpeg', [
						...
						.on('exit', () => {
							const mediaData = fs.readFileSync('../tmp/harta.webp');
							fs.unlink('../tmp/harta.gif', () => {
								fs.unlink('../tmp/harta.webp', () => {});
								fs.emptyDir('../tmp/frame', () => {});
							});
							return resolve(mediaData);
						});
				});
	...
	new Promise(resolve => {
		function render(name: string, text: string, number: number): void {
			return gm()
				...
				.drawText(0, 0, 'HARTA\nTAHTA\n' + text, 'center')
				.wave(4.5, 64)
				.write('../tmp/frame/' + number + '.jpg', () => Promise.resolve());
		}
		switch (number) {
			case 1:
